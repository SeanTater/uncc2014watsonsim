...
public class ChangeFitbAnswerToContentsOfBlanks extends Researcher {
	...
	public void question(Question question, List<Answer> answers) {
		...
		for (Answer a: question) {
			matcher1 = pattern1.matcher(a.direct_passages.get(0).getText()); //title is the text to be searched
			if (matcher1.find()) {	//find the question with blanks (str1) within the document (a.getFullText()); assign result as str1substring (question with blanks filled in)
				...
				String str1substring = a.direct_passages.get(0).getText().substring(docPatternStart, docPatternEnd);
